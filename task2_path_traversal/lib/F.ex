defmodule F do
  ...
  def f(cell_map \\ @cell_map, start \\ 1, goal \\ 9, visited \\ List.duplicate(false,10),path \\ [])
  ...
  #   :nil
  # end
  def f(cell_map , start , goal, visited,path) do
    ...
    # IO.inspect(start)
     ...
     value = for i <- 1..length(Map.fetch!(cell_map,start)) do
      ...
      if Enum.at(visited,next) == false do
        # path ++ [next+1]
        j = f(cell_map,next,goal,visited,path)
        if List.last(j)==goal do
          IO.inspect(j,label: "j found") end
        # end
      # else
      #   path |> Enum.reverse() |> tl() |> Enum.reverse()
      end
